...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction, _ = self.forward(obs, acs, data_statistics["obs_mean"], data_statistics["obs_std"],
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        diff = next_observations - observations
        target = normalize(diff, data_statistics["delta_mean"], data_statistics["delta_std"])
        target = ptu.from_numpy(target)

        _, y_hat = self.forward(observations, actions, data_statistics["obs_mean"], data_statistics["obs_std"],
                                data_statistics["acs_mean"], data_statistics["acs_std"], data_statistics["delta_mean"],
                                data_statistics["delta_std"])
